...
class LocationField extends ModalSelectField
{
	...
	public function setup(\SimpleXMLElement $element, $value, $group = null)
	{
		...
		// Prepare Urls
		$linkLocations = (new Uri())->setPath(Uri::base(true) . '/index.php');
		$linkLocations->setQuery([
			'option'                => 'com_sdajem',
			'view'                  => 'locations',
			'layout'                => 'modal',
			'tmpl'                  => 'component',
		...
		if ($language) {
			...
			$linkLocation->setVar('forcedLanguage', $language);
		...
		$urlSelect = $linkLocations;
		$urlEdit   = clone $linkLocation;
		$urlEdit->setVar('task', 'location.edit');
		$urlNew    = clone $linkLocation;
		...
		$this->modalTitles['new']     = Text::_('COM_SDAJEM_NEW_LOCATION');
		$this->modalTitles['edit']    = Text::_('COM_SDAJEM_EDIT_LOCATION');

		$this->hint = $this->hint ?: Text::_('COM_SDAJEM_SELECT_A_LOCATION');
